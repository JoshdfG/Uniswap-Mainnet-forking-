...
const main = async () => {
  ...
  const usdcBalAfterAddingLiquidity = await USDC.balanceOf(
    impersonatedSigner.address
  );
  const daiBalAfterAddingLiquidity = await DAI.balanceOf(
    impersonatedSigner.address
  );
  ...
  console.log(
    ...
    ethers.formatUnits(usdcBalAfterAddingLiquidity, 6)
  );
  console.log(
    "DAI Balance after swap:",
    ethers.formatUnits(daiBalAfterAddingLiquidity, 18)
  );
